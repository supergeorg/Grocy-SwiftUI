...
struct MDProductFormView: View {
    ...
    init(existingProduct: MDProduct? = nil, userSettings: GrocyUserSettings? = nil) {
        ...
        let initialProduct = existingProduct ?? MDProduct(
            id: 0,
            name: "",
    ...
    var body: some View {
        List {
            if (existingProduct == nil || devMode) && openFoodFactsBarcode == nil {
                Button(action: {
                ...
                .navigationDestination(isPresented: $showOFFResult, destination: {
                    OpenFoodFactsFillProductView(productName: $product.name, queuedBarcode: $queuedBarcode)
                })
            }
            
            if let openFoodFactsBarcode = openFoodFactsBarcode {
                MDProductFormOFFView(barcode: openFoodFactsBarcode, name: $product.name)
            ...
            Section {
                NavigationLink(value: MDProductFormPart.optional, label: {
                ...
                NavigationLink(value: MDProductFormPart.location, label: {
                ...
                NavigationLink(value: MDProductFormPart.dueDate, label: {
                    MyLabelWithSubtitle(title: "Due date", subTitle: "Type, Default days; after opening, freezing, thawing", systemImage: MySymbols.date)
                })
                NavigationLink(value: MDProductFormPart.quantityUnit, label: {
                    MyLabelWithSubtitle(title: "Quantity units", subTitle: "Stock, Purchase", systemImage: MySymbols.quantityUnit, isProblem: (product.quIDStock == 0 || product.quIDPurchase == 0 || product.quIDConsume == 0 || product.quIDPrice == 0))
                })
                NavigationLink(value: MDProductFormPart.amount, label: {
                    MyLabelWithSubtitle(title: "Amounts", subTitle: "Min. stock, Quick consume, Factor, Tare weight", systemImage: MySymbols.amount)
                })
                NavigationLink(value: MDProductFormPart.barcode, label: {
        ...
        .navigationDestination(for: MDProductFormPart.self, destination: { formPart in
            switch formPart {
            case .optional:
    ...
    var optionalPropertiesView: some View {
        Form {
            ...
            MyToggle(isOn: $product.hideOnStockOverview, description: "Never show on stock overview ", descriptionInfo: "The stock overview page lists all products which are currently in-stock or below their min. stock amount - enable this to hide this product there always", icon: MySymbols.stockOverview)
            
            // Disable own stock
            MyToggle(isOn: $product.noOwnStock, description: "Disable own stock", descriptionInfo: "When enabled, this product can't have own stock, means it will not be selectable on purchase (useful for parent products which are just used as a summary/total view of the child products)", icon: MySymbols.stockOverview)
    ...
    var locationPropertiesView: some View {
        Form {
            ...
            if product.defaultConsumeLocationID != nil {
                MyToggle(isOn: $product.moveOnOpen, description: "Move on open", descriptionInfo: "When enabled, on marking this product as opened, the corresponding amount will be moved to the default consume location", icon: MySymbols.transfer)
            }
    ...
    var dueDatePropertiesView: some View {
        Form {
            VStack(alignment: .leading){
                ...
                Picker("", selection: $product.dueType, content: {
            ...
            // Default due days
            MyIntStepper(amount: $product.defaultDueDays, description: "Default due days", helpText: "For purchases this amount of days will be added to today for the due date suggestion (-1 means that this product will be never overdue)", minAmount: -1, amountName: product.defaultDueDays == 1 ? "Day" : "Days", systemImage: MySymbols.date)
            
            // Default due days afer opening
            MyIntStepper(amount: $product.defaultDueDaysAfterOpen, description: "Default due days after opened", helpText: "When this product was marked as opened, the due date will be replaced by today + this amount of days (a value of 0 disables this)", minAmount: 0, amountName: product.defaultDueDaysAfterOpen == 1 ? "Day" : "Days", systemImage: MySymbols.date)
            
            // Default due days after freezing
            MyIntStepper(amount: $product.defaultDueDaysAfterFreezing, description: "Default due days after freezing", helpText: "On moving this product to a freezer location (so when freezing it), the due date will be replaced by today + this amount of days", minAmount: -1, amountName: product.defaultDueDaysAfterFreezing == 1 ? "Day" : "Days", errorMessage: "This cannot be lower than -1 and needs to be a valid number with max. 0 decimal places", systemImage: MySymbols.freezing)
            
            // Default due days after thawing
            MyIntStepper(amount: $product.defaultDueDaysAfterThawing, description: "Default due days after thawing", helpText: "On moving this product from a freezer location (so when thawing it), the due date will be replaced by today + this amount of days", minAmount: 0, amountName: product.defaultDueDaysAfterThawing == 1 ? "Day" : "Days", errorMessage: "This cannot be lower than 0 and needs to be a valid number with max. 0 decimal places", systemImage: MySymbols.thawing)
        }
    ...
    var amountPropertiesView: some View {
        Form {
            ...
            MyToggle(isOn: $product.cumulateMinStockAmountOfSubProducts, description: "Accumulate sub products min. stock amount ", descriptionInfo: "If enabled, the min. stock amount of sub products will be accumulated into this product, means the sub product will never be \"missing\", only this product", icon: MySymbols.accumulate)
            
            // Treat opened as out of stock
            MyToggle(isOn: $product.treatOpenedAsOutOfStock, description: "Treat opened as out of stock", descriptionInfo: "When enabled, opened items will be counted as missing for calculating if this product is below its minimum stock amount", icon: MySymbols.stockOverview)
            
            // Quick consume amount
            MyDoubleStepperOptional(amount: $product.quickConsumeAmount, description: "Quick consume amount", descriptionInfo: "This amount is used for the \"quick consume button\" on the stock overview page (related to quantity unit stock)", minAmount: 0.0001, amountStep: 1.0, amountName: nil, systemImage: MySymbols.consume)
            
            // Quick open amount
            MyDoubleStepperOptional(amount: $product.quickOpenAmount, description: "Quick open amount", descriptionInfo: "This amount is used for the \"quick open button\" on the stock overview page (related to quantity unit stock)", minAmount: 0.0001, amountStep: 1.0, amountName: nil, systemImage: MySymbols.open)
            
            // Tare weight
            Group {
                MyToggle(isOn: $product.enableTareWeightHandling, description: "Enable tare weight handling", descriptionInfo: "This is useful e.g. for flour in jars - on purchase/consume/inventory you always weigh the whole jar, the amount to be posted is then automatically calculated based on what is in stock and the tare weight defined below", icon: MySymbols.tareWeight)
            ...
            MyToggle(isOn: $product.notCheckStockFulfillmentForRecipes, description: "Disable stock fulfillment checking for this ingredient", descriptionInfo: "This will be used as the default setting when adding this product as a recipe ingredient", icon: MySymbols.recipe)
        }
